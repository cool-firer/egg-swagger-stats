...
// Count request
swsTimeline.prototype.countRequest = function (ctx) {

    // Count in timeline
    this.getTimelineBucket(ctx.sws.timelineid).stats.countRequest(ctx.sws.req_clength);
};

// Count finished response
swsTimeline.prototype.countResponse = function (ctx) {

    // Update timeline stats
    this.getTimelineBucket(ctx.sws.timelineid).stats.countResponse(
                    ctx.res.statusCode,
                    swsUtil.getStatusCodeClass(ctx.res.statusCode),
                    ctx.sws.duration,
                    ctx.sws.res_clength);
