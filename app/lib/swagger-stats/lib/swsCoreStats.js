...
// Count request
swsCoreStats.prototype.countRequest = function (ctx) {

    // Count in all
    this.all.countRequest(ctx.sws.req_clength);
    ...
    this.method[method].countRequest(ctx.sws.req_clength);
...
swsCoreStats.prototype.countResponse = function (ctx) {
    ...
    if ("_contentLength" in ctx.res && ctx.res['_contentLength'] !== null ){
        resContentLength = ctx.res['_contentLength'];
    }else{
        // Try header
        if(ctx.res.getHeader('content-length') !==null ) {
            resContentLength = parseInt(ctx.res.getHeader('content-length'));
        }
    }

    if("sws" in ctx) {
        startts = ctx.sws.startts;
        timelineid = ctx.sws.timelineid;
        var endts = Date.now();
        ctx['sws'].endts = endts;
        duration = endts - startts;
        ctx['sws'].duration = duration;
        ctx['sws'].res_clength = resContentLength;
        path = ctx['sws'].api_path;
    }
    ...
    this.all.countResponse(ctx.res.statusCode,codeclass,duration,resContentLength);

    // Update method-specific stats
    var method = ctx.method;
    if (method in this.method) {
        var mstat = this.method[method];
        mstat.countResponse(ctx.res.statusCode,codeclass,duration,resContentLength);
    }
